/*USAGE: compara_kernels –N=<dim_mat> -W=<dim_bloq>*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
int run_program(int N, int W)
{
    ...
    for(int i = 0; i < warmup_iters; i++) {
        kernel_run_chrono(N, grid, block, A_d, B_d, C_d, tile_dim);
    }

    // Benchmark iterations
    float total_time = 0.0f;
    for(int i = 0; i < bench_iters; i++) {
        total_time += kernel_run_chrono(N, grid, block, A_d, B_d, C_d, tile_dim);
    }

    float avg_time = total_time / bench_iters / 1000; //average time in seconds

    // GFLOPS/s=(2N^3/avg_matmult_time)/1e9
    float Gflops = ((2. * N * N * N) / (avg_time)) / 1000000000.;
    printf("%d,%d,%f\n", N, W, Gflops);
