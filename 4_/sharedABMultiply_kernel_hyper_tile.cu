__global__ void sharedABMultiply(float *a, float* b, float *c, int N, int K)
{
    ...
    for (int i = 0; i < R; i++) // Loop over the tiles of the matrices
    {
        ...
        //print all indexes themselves in aTile and bTile and a and b
        //print i, threadIdx.x, threadIdx.y, row, col
        //printf("i = %d, threadIdx.x = %d, threadIdx.y = %d, row = %d, col = %d\n", i, threadIdx.x, threadIdx.y, row, col);
        //printf("a[%d] = %f, b[%d] = %f\n", row * K + i * tile_dim + threadIdx.x, a[row * K + i * tile_dim + threadIdx.x], (i * tile_dim + threadIdx.y) * K + col, b[(i * tile_dim + threadIdx.y) * K + col]);
